...
const Contact = forwardRef(function Contact(_, ref) {
  ...
  // ── Client-side validation (mirrors the backend rules exactly) ──
  const validate = () => {
    const errs = [];
  ...
  const onSubmit = async e => {
    ...
    // Validate on the client first — stops pointless 422s
    const errs = validate();
    if (errs.length) {
      setErrMsgs(errs);
      setStatus("error");
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Parse the backend error messages if available
        try {
          const data = await res.json();
          const msgs = data.messages || (data.message ? [data.message] : ["Something went wrong."]);
          setErrMsgs(msgs);
        } catch {
          setErrMsgs(["Something went wrong. Please try again."]);
        }
        setStatus("error");
      }
    } catch {
      setErrMsgs(["Network error — make sure the server is running."]);
      setStatus("error");
    }
  };
  ...
  return (
    <section ref={ref} className="page-section"
      ...
      <p style={{ fontSize:"14px", color:"#c4a0e8", maxWidth:"480px", textAlign:"center", lineHeight:1.75, marginBottom:"1.8rem" }}>
      ...
      <div className="contact-cards">
        {CARDS.map(c => (
          <a key={c.label} href={c.href} target="_blank" rel="noreferrer" className="contact-card">
            <div style={{ color:"#f0c040", display:"flex", justifyContent:"center", marginBottom:"8px" }}><c.Icon /></div>
            <div style={{ fontFamily:"'Cinzel',serif", fontSize:"9px", letterSpacing:"0.15em", color:"#8060a0", marginBottom:"6px" }}>{c.label}</div>
            <div style={{ fontSize:"11px", color:"#c070ff", wordBreak:"break-all" }}>{c.val}</div>
          </a>
        ))}
      </div>
      ...
      <form onSubmit={onSubmit} className="cf-form" noValidate>
        ...
        {/* Success */}
        {status === "success" && (
          <p style={{ fontFamily:"'Cinzel',serif", fontSize:"11px", letterSpacing:"0.15em",
            color:"#50fa7b", textAlign:"center", marginBottom:"10px" }}>
            ✦ MESSAGE SENT — I'LL REPLY WITHIN 24 HOURS ✦
          </p>
        )}

        {/* Errors — show each message from backend or client validation */}
        {status === "error" && errMsgs.length > 0 && (
          <div style={{ marginBottom:"10px" }}>
            {errMsgs.map((msg, i) => (
              <p key={i} style={{ fontFamily:"'Cinzel',serif", fontSize:"11px", letterSpacing:"0.1em",
      ...
      <p style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:"11px", color:"#8060a0", marginTop:"2rem", textAlign:"center" }}>
