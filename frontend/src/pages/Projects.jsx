...
const Projects = forwardRef(function Projects(_, ref) {
  ...
  return (
    <section ref={ref} className="page-section"
      ...
      {loading ? <p style={{ fontFamily:"'Cinzel',serif", color:"#8060a0" }}>LOADING...</p>
        : <div className="proj-grid">
            {projects.map(p => (
              <div key={p.id} className="proj-card">
                {/* Thumbnail */}
                <div className="proj-thumb" style={{ background:p.grad }}>
                  <div className="proj-thumb-icon" style={{ color:p.iconColor }}>
                    <p.Icon />
                  </div>
                ...
                <div style={{ padding:"1.3rem 1.5rem" }}>
                  <span style={{ display:"inline-block", background:"rgba(240,192,64,0.1)",
                    ...
                    padding:"3px 8px", borderRadius:"4px", marginBottom:"8px" }}>{p.tag}</span>
                  <div style={{ fontFamily:"'Cinzel',serif", fontSize:"15px", color:"#f5e6ff", fontWeight:600, marginBottom:"4px" }}>{p.name}</div>
                  <div style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:"10px", color:"#8060a0", marginBottom:"12px" }}>
                    <svg viewBox="0 0 24 24" width="11" height="11" fill="none" stroke="#8060a0" strokeWidth="2" style={{ display:"inline", marginRight:"4px", verticalAlign:"middle" }}>
                  ...
                  <ul style={{ listStyle:"none", padding:0, marginBottom:"14px" }}>
                    {p.bullets.map((b,i) => (
                      <li key={i} style={{ fontSize:"12px", color:"#c4a0e8", paddingLeft:"14px", position:"relative", lineHeight:1.6, marginBottom:"5px" }}>
                        <span style={{ position:"absolute", left:0, color:"#f0c040", fontSize:"8px", top:"3px" }}>◆</span>{b}
                      </li>
                    ))}
                  </ul>
                  <div style={{ display:"flex", gap:"8px" }}>
                    <a href={p.github} target="_blank" rel="noreferrer"
                      style={{ fontFamily:"'Cinzel',serif", fontSize:"9px", fontWeight:600, letterSpacing:"0.1em",
                      ...
                      <svg viewBox="0 0 24 24" width="11" height="11" fill="#f0c040">
